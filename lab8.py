...
def rod_cutter_with_values(values, n):
    D = rod_cutter_dic(values,{})
    return(rod_cutter(values,n), rod_cutter_values(values,n,D))
    pass
def rod_cutter_dic(values,D):
    if values == []:
        return D
    else:
        D[(values[0][0])] = values[0][1]
        return rod_cutter_dic(values[1:],D)
def rod_cutter_values(values,n,D):
    if values == [] or n == 0:
        return []
    elif values[0][0] > n:
        return rod_cutter_values(values[1:],n,D)
    else:
        useIt = rod_cutter_values(values,n - values[0][0],D) + [values[0][0]]
        loseIt = rod_cutter_values(values[1:],n,D)
        if useIt == []:
            return loseIt
        elif loseIt == []:
            return useIt
        elif reduce(lambda x,y: x+y ,map(lambda x:D[(x)],useIt)) > reduce(lambda x,y: x+y ,map(lambda x:D[(x)],loseIt)):
            return useIt
        else:
            return loseIt
