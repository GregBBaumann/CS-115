...
def letterScoreLoop(letter, scorelist):
    '''Same as letterScore'''
    for place in range(len(scorelist)):
        if letter == scorelist[place][0]:
            return scorelist[place][1]
...
def wordScoreLoop(S, scorelist):
    ...
    for letters in range(len(S)):
        score += letterScoreLoop(S[letters],scorelist)
    return score
    pass  # TODO
...
def wordsWithScoreLoop(dct, scores):
    ...
    if dct == []:
        return []
    for pos in range(len(dct)):
        dct[pos] = [dct[pos],wordScoreLoop(dct[pos],scores)]
    return dct
    pass  # TODO
